...
================================================================================
 ...
 Changes:
    ...
    09/03/2024 1.8.1 graphics marker scaling by aurrent zoom
*/

#feature-id    salvolm > GAME
...
function maskData(view, callback)
{
   ...
   */
   ...
   this.getMaskView = function ()
   {
      ...
      mask.beginProcess(UndoFlag.NoSwapFile);
...
function showDialog_init(cview, Id)
{
   ...
   var strSettings = Settings.read(ID, DataType.String );
   ...
   this.previewControl.onCustomMouseDown = function (x, y, button, buttonState, modifiers)
   {
      ...
      else if (data.objectType == 1)
      {
         ...
         if (data.currentCurve.selected && modifiers == KeyModifier.Control) return;
   ...
   this.previewControl.onCustomMouseMove = function (x, y, buttonState, modifiers)
   {
      ...
      else if (data.objectType == 1)
      {
         ...
         if (data.currentCurve != null)
         {
            ...
            {
               if (data.currentCurve.selected && modifiers == KeyModifier.Control)
               {
         ...
         if (buttonState == MouseButton.Unknown)
         {
            for (var i = 0; i < data.multiPoints.length; i++)
            {
               if (!data.currentCurve == null)
   ...
   with (this)
   {
      ...
      onKeyPress = function(k, m)
      {
         ...
         else if (k == 27)
         {
            ...
            {
               if (ask("Figure(s) created, save before exit?","Warning" , StdButton.Yes))
               {
                  createMask(data,
                                dialog.view,
                                dialog.id,
...
function createMask(data, view, id, settings, gradientMask, lightMask, plainMask, brightMask, starMask, gradientedgeMask)
{
   ...
   else
   {
      ...
      for (var i = 0; i < createdMasks.length; i++)
         {
            // V8: View.viewById returns null (not an invalid View) when the
            // id cannot be resolved. Skip gracefully instead of crashing on
            // .id / .window access.
            var v = View.viewById( createdMasks[i] );
            if (v == null || v.isNull)
            {
               Console.warningln('Mask view not found: ' + createdMasks[i]);
               continue;
            }
            Console.writeln('\nfinish mask ' + v.id);
...
function ellipsoid(x, y, a, b, pa, caller)
{
   ...
   this.draw = function (g)
   {
      ...
      var radius = settings.apDiameter / 2 / zoom;
      //
      if (settings == null) return;
      // var radius = settings.apDiameter / 2 / zoom;
      var lineColor;
      var lineStyle = PenStyle.Solid;
      if (!settings.afLineStyle) lineStyle = PenStyle.Dash;
      var centerColor;
      if (this.selected)
      {
      ...
      // center point
      //####################
      //  removed 09/03/2024: radius = settings.cpDiameter / 2;
...
function getNewName(name, suffix)
{
   ...
   let n = 1;
   // V8: ImageWindow.windowById returns null when no window with that id
   // exists, where the legacy SpiderMonkey runtime returned an invalid
   // window with .isNull === true. Treat both as "the id is free".
   for (;;)
   {
      var w = ImageWindow.windowById(newName);
      if (w == null || w.isNull)
         break;
      ++n;
...
function loadMaskFromView( view, data, ctrl )
{
   ...
   if (old_ellipsoids.length > 0)
   {
      ...
      {
         if (!ask("Erase figure(s) on active view?", "Confirm", StdButton.Yes))
            return;
      }
...
function ask(txt, caption, defaultButton)
{
   ...
   return bx.execute() == StdButton.Yes;
}
...
function PreviewControl_init(parent)
{
   ...
   {
      // next line changed from 2 to 10 on Sep. 02/2024 to get higher zoom limit
      newZoom = Math.max(this.zoomOutLimit, Math.min(/*2*/ 10, newZoom));
...
function showOptions_init(strSettings)
{
   var dialog = this;
...
function showSelectiveRejection_init(data)
{
   var dialog = this;
...
function applyToViews(viewIds, mask)
{
   ...
   for (var i = 0; i < viewIds.length;i++)
   {
      ...
      {
         protocol.push('View not found: ' + viewIds[i]);
         continue;
      }
...
//
function showSelectMainviews_init()
{
   var dialog = this;
...
function defaultSettings()
{
   var defs =
   {
      ...
      // Active figure line
      afLineStyle:   true,          // PenStyle.Solid | PenStyle.Dash
...
class PreviewControl extends Frame {
   constructor(parent) {
      super(parent);
      PreviewControl_init.call(this, parent);
   }
}

class showDialog extends Dialog {
   constructor(cview, Id) {
      super();
      showDialog_init.call(this, cview, Id);
   }
}

class showOptions extends Dialog {
   constructor(strSettings) {
      super();
      showOptions_init.call(this, strSettings);
   }
}

class showSelectiveRejection extends Dialog {
   constructor(data) {
      super();
      showSelectiveRejection_init.call(this, data);
   }
}

class showSelectMainviews extends Dialog {
   constructor() {
      super();
      showSelectMainviews_init.call(this);
   }
}
